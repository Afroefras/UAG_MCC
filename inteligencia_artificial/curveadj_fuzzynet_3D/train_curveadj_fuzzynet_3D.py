...
class CurveAdjFuzzyNet3D:
    ...
    def set_fuzzynet(self) -> None:
        ...
        for i in range(6):
            var = '"x"' if i < 3 else '"y"'
            self.func[f"mf{i+1}"] = (
                f'exp(-({var} - {self.func[f"M{i+1}"]})**2 / 2*{self.func[f"D{i+1}"]}**2)'
            )
        ...
        for i in range(3):
            for j in range(3, 6):
                self.func[f"inf{n}"] = (
                    f"({self.func[f'mf{i+1}']})*({self.func[f'mf{j+1}']})"
                )
                self.func[
                    f"reg{n}"
                ] = f"""
        ...
        self.func["a"] = [
            y for x, y in self.func.items() if search("reg", x) is not None
        ]
        self.func["a"] = " + ".join(self.func["a"])

        self.func["b"] = [
            y for x, y in self.func.items() if search("inf", x) is not None
        ]
    ...
    def get_population_surfaces(self) -> None:
        ...
        for indiv in self.population:
            self.pop_surfaces.append(
                self.surface_values(
                    self.function_to_eval(self.func_string, values_list=indiv)
                )
            )
